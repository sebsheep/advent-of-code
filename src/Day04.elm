module Day04 exposing (Event, res1, res2)
...
res1 =
    Parser.run listEventParser input
        |> Result.map shifParser
        |> Result.map idGuardAndMinute


res2 =
    Parser.run listEventParser input
        |> Result.map shifParser
        |> Result.map countByGuard



--        |> Result.map greatestIntervalInNights


type alias Night =
    { idGuard : Int
    , asleepedIntervals : List TimeInterval
    }
...
countByGuard : List Night -> Maybe ( Int, Maybe ( Guard, Int ) )
countByGuard nights =
    List.foldr
        (\night counter ->
            night.asleepedIntervals
                |> List.map toMinutesList
                |> List.concat
                |> List.foldr
                    (\minute innerCounter ->
                        update minute (incrementD night.idGuard) innerCounter
                    )
                    counter
        )
        (Array.repeat 60 Dict.empty)
        nights
        ...
        |> Maybe.map
            (Tuple.mapSecond
                (Dict.toList >> List.maximumBy Tuple.second)
            )


sleepingTime : List TimeInterval -> Int
sleepingTime intervals =
    intervals
        |> List.map .duration
        |> List.sum


idGuardAndMinute : List Night -> ( Maybe Int, Maybe Int )
idGuardAndMinute nights =
    let
        idGuardM =
            mostAsleeped nights
    in
    ( idGuardM
    , idGuardM |> Maybe.andThen (minuteMostAsleeped nights)
    )


mostAsleeped : List Night -> Maybe Int
mostAsleeped nights =
    List.foldr
        (\night acc ->
            case Dict.get night.idGuard acc of
                Nothing ->
                    Dict.insert night.idGuard (sleepingTime night.asleepedIntervals) acc

                Just n ->
                    Dict.insert night.idGuard (n + sleepingTime night.asleepedIntervals) acc
        )
        Dict.empty
        nights
        |> Dict.toList
...
incrementA : Int -> Array Int -> Array Int
incrementA i array =
...
update i f array =
    case Array.get i array of
        ...
        Just x ->
            Array.set i (f x) array


incrementD : comparable -> Dict comparable Int -> Dict comparable Int
incrementD i dict =
    case Dict.get i dict of
        Nothing ->
            Dict.insert i 1 dict

        Just n ->
            Dict.insert i (n + 1) dict


toMinutesList : TimeInterval -> List Int
toMinutesList { begin, duration } =
    List.range begin (begin + duration)


minuteMostAsleeped : List Night -> Int -> Maybe Int
minuteMostAsleeped nights idGuard =
    List.filter (\n -> n.idGuard == idGuard) nights
        |> List.map .asleepedIntervals
        |> List.concat
        |> List.foldr
            (\interval minutes ->
                List.foldr incrementA minutes (toMinutesList interval)
            )
            (Array.repeat 60 0)
        |> Array.indexedMap Tuple.pair
        |> Array.toList
...
--|> Maybe.map (Tuple.mapSecond .begin)


type ShiftParser
    = Begin
    | BuildingNight (List Night) Night
    | AddingIntervall (List Night) Night Int


stepShiftParser : Event -> ShiftParser -> ShiftParser
stepShiftParser event state =
    case ( event.kind, state ) of
        ( BeginShift idGuard, Begin ) ->
            BuildingNight [] { idGuard = idGuard, asleepedIntervals = [] }

        ( FallsAsleep, BuildingNight nights night ) ->
            AddingIntervall nights night event.minutes

        ( WakesUp, AddingIntervall nights night begin ) ->
            let
                newInter =
                    { begin = begin, duration = event.minutes - begin }
            in
            BuildingNight nights
                { night
                    | asleepedIntervals =
                        newInter :: night.asleepedIntervals
                }

        ( BeginShift idGuard, BuildingNight nights night ) ->
            BuildingNight (night :: nights) { idGuard = idGuard, asleepedIntervals = [] }

        ( _, _ ) ->
            Debug.todo "Error during step:" (Debug.toString ( event, state ))


innerShiftParser : List Event -> ShiftParser -> List Night
innerShiftParser events state =
    -- the events list is supposed to be sorted
    case ( events, state ) of
        ( [], Begin ) ->
            []

        ( [], BuildingNight nights night ) ->
            night :: nights

        ( [], _ ) ->
            Debug.todo "Error during inner, empty events with state:" state

        ( event :: nextEvents, _ ) ->
            innerShiftParser nextEvents (stepShiftParser event state)


shifParser : List Event -> List Night
shifParser events =
    let
        sortedEvents =
            events
                |> List.sortBy
                    (\e -> [ e.year, e.month, e.day, e.hours, e.minutes ])
    in
    innerShiftParser sortedEvents Begin
...
eventKindParser : Parser EventKind
eventKindParser =
    Parser.oneOf
        [ Parser.map (\_ -> WakesUp) (Parser.symbol "wakes up")
        , Parser.map (\_ -> FallsAsleep) (Parser.symbol "falls asleep")
        , Parser.succeed BeginShift
            |. Parser.symbol "Guard #"
            |= Parser.int
...
listEventParser : Parser (List Event)
listEventParser =
    Parser.sequence
        { start = ""
        , separator = ""
